...
A Windows 11 desktop application for host hardening, BitLocker management, WDAC policy control, event-log inspection, network visibility, and multi-engine file scanning that combines **YARA**, **Sigma**, and **LLM-powered script analysis**.
...
- The `%ProgramData%\TGWST` directory structure (WDAC data, models, cache).
- First-run bootstrap wizard trigger (runs on first launch to provision WSL2/Ubuntu, Docker, Pi-hole, DNS sync, and sudo credential caching).
...
### Fresh Machine Provisioning (Post-Install)
On a new Windows 11 machine, launch TGWST after MSI installation and complete the **First-Run Provisioning Wizard**.  
This wizard is designed to handle:
- WSL2 + Ubuntu distro readiness
- Docker engine installation in Ubuntu
- Pi-hole appliance installation and WSL-IP-aware startup service
- Optional Windows DNS sync to WSL Pi-hole endpoint
- Encrypted sudo credential caching (DPAPI) for background privileged operations
